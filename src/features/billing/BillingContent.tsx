...
export default function BillingContent({ embedded = false }: BillingContentProps) {
  const { user } = useAuth();
  ...
  const [selectedTopUp, setSelectedTopUp] = useState<ReturnType<typeof getTopUpPackages>[number] | null>(null);
  ...
  const tiers = [
    { id: 'entry', name: 'Entry', price: 19, corePT: 300, advancedPT: 'Add-on only', features: ['300 Core Points/month', '5 AI agents', 'Core models', 'Email support (48h)'] },
    { id: 'pro', name: 'Pro', price: 49, corePT: 1000, advancedPT: '50 Points metered', features: ['1,000 Core Points/month', '50 Advanced Points', '25 AI agents', 'All models', 'API access', 'Email support (24h)'], popular: true },
    { id: 'proplus', name: 'Pro Plus', price: 79, corePT: 1600, advancedPT: '100 Points metered', features: ['1,600 Core Points/month', '100 Advanced Points', '50 AI agents', 'Priority access', 'Team workspaces (3)', 'Email + chat support (12h)'] },
    { id: 'business', name: 'Business', price: 159, corePT: 3500, advancedPT: '200 Points pools', features: ['3,500 Core Points/month', '200 Advanced Points', '100 AI agents', 'Dedicated capacity', 'Unlimited workspaces', 'Priority support (4h SLA)'] },
  ];
  ...
  return (
    <div className={embedded ? '' : 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8'}>
      ...
      {ptData && (
        <div className="mb-8">
          <PTHealthBar
            corePT={{ used: ptData.core.used, total: ptData.core.total, percentage: ptData.core.percentage }}
            advancedPT={{ used: ptData.advanced.used, total: ptData.advanced.total, percentage: ptData.advanced.percentage }}
            tier={ptData.tier}
          />
        </div>
      )}
        ...
        <div className="dashboard-card mb-8 p-6">
          <div className="flex items-start justify-between mb-6">
            ...
            {subscription.tier !== 'freemium' && (
              <button onClick={handleManagePayment} disabled={actionLoading === 'portal'} className="btn-system">
          ...
          {subscription.tier !== 'freemium' && (
            <div className="border-t pt-6 mt-6" style={{ borderColor: 'var(--card-border)' }}>
              <button onClick={handleCancelSubscription} disabled={actionLoading === 'cancel'} className="text-sm font-medium" style={{ color: '#ff6b6b' }}>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {topUpPackages.map((pkg) => (
            <div key={pkg.id} className={`dashboard-card p-6 ${pkg.popular ? 'ring-2 ring-purple-500/30' : ''}`}>
              {pkg.popular && (
              ...
              <button onClick={() => { setSelectedTopUp(pkg); setShowTopUpModal(true); }} disabled={actionLoading === `topup-${pkg.ptAmount}`} className="w-full btn-system">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-md rounded-xl dashboard-card">
            <div className="px-6 py-4 border-b flex items-center justify-between" style={{ borderColor: 'var(--card-border)' }}>
              ...
              <button className="opacity-80 hover:opacity-100" onClick={() => { setShowTopUpModal(false); setSelectedTopUp(null); }} aria-label="Close">×</button>
            </div>
            ...
            <div className="px-6 py-4 border-t flex items-center justify-end gap-3" style={{ borderColor: 'var(--card-border)' }}>
              <button className="px-4 py-2 rounded-lg" style={{ background: 'var(--panel-bg)', color: 'var(--text)' }} onClick={() => { setShowTopUpModal(false); setSelectedTopUp(null); }}>
                Cancel
              </button>
              <button onClick={handleTopUpConfirm} disabled={actionLoading === `topup-${selectedTopUp.ptAmount}`} className="px-4 py-2 rounded-lg btn-gold disabled:opacity-50">
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {tiers.map((tier) => {
            ...
            return (
              <div key={tier.id} className={`dashboard-card p-6 ${tier.popular ? 'ring-2 ring-purple-500/30' : ''}`}>
                {tier.popular && (
                  <span className="inline-block px-2 py-1 text-xs font-semibold rounded mb-3" style={{ background: 'var(--accent-muted)', color: 'var(--accent-gold)' }}>
                    Most Popular
                  </span>
                ...
                <div className="mb-4">
                  <div className="text-sm font-medium mb-1" style={{ color: 'var(--text)' }}>{tier.corePT} Core Points</div>
                  <div className="text-sm" style={{ color: 'var(--muted)' }}>{tier.advancedPT.replace('PT', 'Points')}</div>
                </div>
                ...
                <button onClick={() => handleUpgrade(tier.id)} disabled={isCurrent || actionLoading === `upgrade-${tier.id}`} className={`w-full py-2 px-4 rounded-lg font-medium transition-colors ${isCurrent ? 'opacity-50 cursor-not-allowed card-surface' : 'btn-system'}`}>
        ...
        <div className="dashboard-card p-6">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead>
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider" style={{ color: 'var(--muted)' }}>Date</th>
                  <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider" style={{ color: 'var(--muted)' }}>Description</th>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider" style={{ color: 'var(--muted)' }}>Invoice</th>
                </tr>
              </thead>
              <tbody>
                {billingHistory.map((invoice) => (
                  <tr key={invoice.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 text-xs font-semibold rounded-full`} style={{ background: 'var(--accent-muted)', color: 'var(--accent-gold)' }}>
                        {invoice.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      {invoice.invoice_url && (
                        <a href={invoice.invoice_url} target="_blank" rel="noopener noreferrer" className="flex items-center" style={{ color: 'var(--accent-gold)' }}>
